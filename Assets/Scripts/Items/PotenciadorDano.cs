...
public class PotenciadorDano : MonoBehaviour {

    //Este script debe colocarse sobre un objeto que actúe como un item del juego
    //Cuando el personaje con la etiqueta Player toca el objeto aumentará de tamaño
    //y aumentará el daño que provoca. Las balas también serñan más grandes mientras dure este efecto
    ...
    void Update () {
        ...
        if (isPlayer)
        {
            //Aumentamos el tamaño del PJ con un Lerp
            padre.localScale = new Vector3(Mathf.Lerp(1f, 1.3f, Time.time), Mathf.Lerp(1f, 1.3f, Time.time), Mathf.Lerp(1f, 1.3f, Time.time));
        ...
        if (tiempoTranscurrido > 4.9f && tiempoTranscurrido < 5.1f)
        {
            ...
            padre.localScale = new Vector3(Mathf.Lerp(1.3f, 1f, Time.time), Mathf.Lerp(1.3f, 1f, Time.time), Mathf.Lerp(1.3f, 1f, Time.time));
            //Quitamos el daño extra
    ...
    void OnTriggerEnter(Collider info)
    {
        ...
        //Pasamos la cantidad de daño extra que realizará
        tankShooting.bonusDamage = extraDamage;
        //Indicamos con 1 que las balas aumenten de tamaño
